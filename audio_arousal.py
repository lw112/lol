...
y_arous = y['Merged Arousal'].values
...
checkpointer = ModelCheckpoint(filepath="weights_arousal_MS.hdf5", monitor='val_mean_squared_error', verbose=1, save_best_only=True)
...
model.load_weights("weights_arousal_MS.hdf5")
preds = model.predict([x_test, gender_test, length_test])
#np.savetxt('Arousal_audio_LSTM.txt', y_test, delimiter=',')
np.savetxt('Arousal_audio_predictions_LSTM_ms.txt', preds, delimiter=',')
...
try:
    f = open('Arousal_audio_multistream_optimised.txt','w+')
