...
Dominace_lables = y['Merged Dominance'].values
# integer encode
label_encoder = LabelEncoder()
Dominace_lables_encoded = label_encoder.fit_transform(Dominace_lables)
...
# For arousal
y_train = Dominace_lables_encoded[train_indexes]
y_test = Dominace_lables_encoded[test_indexes]
...
np.savetxt('Results/Image/Static Feature/Image_points_Dominance_predictions_LSTM_ms.txt', preds, delimiter=',')
...
try:
    f = open('Results/Image/Static Feature/Image_points_Dominance_LSTM_results_ms.txt','w+')
...
preds = model.predict([X_test_reshape,clip_lengths_test])
np.savetxt('Results/Image/No Static Feature/Image_points_Dominance_predictions_LSTM.txt', preds, delimiter=',')
preds = preds.reshape(preds.shape[0])
...
try:
    f = open('Results/Image/No Static Feature/Image_points_Dominance_LSTM_results.txt','w+')
    f.write('Mean of predictions is: ')
    mean_y = np.mean(preds)
    f.write(str(mean_y))
    f.write('\nVariance is: ')
